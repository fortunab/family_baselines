"""
Main CLI Entrypoint for fastai Colorectal Histology with TOML Config & Weights & Biases (W&B).
"""
...
from src.fastai_engine import train_fastai_model
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="fastai + TOML + W&B Colorectal Histology Benchmark"
    )
    parser.add_argument(
        "--config", type=str, default="configs/default.toml", help="Path to TOML configuration file"
    )
...
def main():
    ...
    exp_name = f"fastai_{backbone_name}_{active_seed}"
    wandb_enabled = bool(cfg.get("wandb.enabled", True)) and not args.no_wandb
    wandb_project = str(cfg.get("wandb.project", cfg.get("project", "colorectal-histology-fastai")))
    wandb_tags = cfg.get("wandb.tags", ["fastai", "histology", "toml", backbone_name])
    ...
    metrics, y_true, y_pred, y_prob = train_fastai_model(
        train_df=train_df, val_df=val_df, test_df=test_df, cfg=cfg, tracker=tracker
    )

    # 5. Persist Metrics & Visualizations
    cm_path = results_dir / f"confusion_matrix_{backbone_name}.png"
    roc_path = results_dir / f"roc_curves_{backbone_name}.png"
    json_path = results_dir / f"metrics_{backbone_name}.json"

    plot_confusion_matrix(
        y_true, y_pred, output_path=cm_path, title=f"fastai {backbone_name} Confusion Matrix"
    )
    plot_multiclass_roc(
        y_true, y_prob, output_path=roc_path, title=f"fastai {backbone_name} Multi-Class ROC"
    )
