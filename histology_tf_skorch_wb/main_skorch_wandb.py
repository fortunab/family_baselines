"""
Main CLI Entrypoint for skorch Colorectal Histology with TOML Config & Weights & Biases (W&B).
"""
...
from src.skorch_engine import train_skorch_model
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="skorch + TOML + W&B Colorectal Histology Benchmark"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    exp_name = f"skorch_{backbone_name}_{active_seed}"
    wandb_enabled = bool(cfg.get("wandb.enabled", True)) and not args.no_wandb
    wandb_project = str(cfg.get("wandb.project", cfg.get("project", "colorectal-histology-skorch")))
    wandb_tags = cfg.get("wandb.tags", ["skorch", "histology", "toml", backbone_name])
    ...
    metrics, y_true, y_pred, y_prob = train_skorch_model(
        train_df=train_df,
        val_df=val_df,
        test_df=test_df,
        cfg=cfg,
    ...
    plot_confusion_matrix(
        y_true, y_pred, output_path=cm_path, title=f"skorch {backbone_name} Confusion Matrix"
    )
    plot_multiclass_roc(
        y_true, y_prob, output_path=roc_path, title=f"skorch {backbone_name} Multi-Class ROC"
    )
