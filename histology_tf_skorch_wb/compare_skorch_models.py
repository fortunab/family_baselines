"""
Leaderboard Comparison & Visualization Script for skorch + TOML + W&B Suite.
Compares:
...
def print_comparison_table(models_data: List[Dict[str, Any]]):
    print("\n" + "=" * 145)
    print("        SKORCH + TOML + W&B FOUNDATION BENCHMARK LEADERBOARD: COLORECTAL HISTOLOGY")
    print("=" * 145)
...
def plot_side_by_side_bars(models_data: List[Dict[str, Any]], output_path: Path):
    ...
    plt.xlabel("skorch Foundation Model Architecture", fontsize=11, weight="bold")
    plt.ylabel("Clinical Performance (%)", fontsize=11, weight="bold")
    plt.title(
        "Colorectal Histology 8-Class Benchmark Leaderboard (skorch + TOML + W&B)",
        fontsize=13,
        weight="bold",
    )
...
def main():
    ...
    plot_side_by_side_bars(models_data, results_dir / "skorch_toml_benchmark_comparison.png")
