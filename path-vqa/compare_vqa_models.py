"""
Comparison & Leaderboard Script for Foundation VQA Models on flaviagiammarino/path-vqa.
Compares:
1. Microsoft BiomedCLIP / Florence-2
...
def print_comparison_table(bio_m, pali_m, qwen_m, blip_m):
    print("\n" + "="*135)
    print("      PATH-VQA FOUNDATION MODEL BENCHMARK: 4-MODEL COMPARISON LEADERBOARD")
    print("="*135)

    header = f"{'Metric':<32} | {'1. BiomedCLIP/Flor2':<22} | {'2. PaliGemma':<22} | {'3. Qwen2-VL':<22} | {'4. BLIP-2':<22}"
    print(header)
    print("-"*135)
    ...
    for label, key, is_pct in metrics_to_compare:
        b_str = format_val(bio_m, key, is_pct)
        p_str = format_val(pali_m, key, is_pct)
        q_str = format_val(qwen_m, key, is_pct)
        l_str = format_val(blip_m, key, is_pct)
        print(f"{label:<32} | {b_str:<22} | {p_str:<22} | {q_str:<22} | {l_str:<22}")

    print("-"*135)
    print("EXPERIMENT SEEDS USED:")
    def format_seed(m): return str(m.get("Experiment_Seed", "N/A")) if m else "N/A"
    print(f"{'Active Random Seed':<32} | {format_seed(bio_m):<22} | {format_seed(pali_m):<22} | {format_seed(qwen_m):<22} | {format_seed(blip_m):<22}")
    print("="*135 + "\n")


def plot_side_by_side_bars(bio_m, pali_m, qwen_m, blip_m, output_path: Path):
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)

    models = ["1. BiomedCLIP", "2. PaliGemma", "3. Qwen2-VL", "4. BLIP-2"]
    metrics_data = [bio_m, pali_m, qwen_m, blip_m]
    ...
    plt.bar(x - width, accs, width, label='Overall VQA Accuracy (EM %)', color='#1f77b4')
    plt.bar(x, yn_accs, width, label='Yes/No Accuracy (%)', color='#2ca02c')
    plt.bar(x + width, rouges, width, label='ROUGE-L F1 Score (%)', color='#ff7f0e')

    plt.xlabel('Multimodal Foundation Model Architecture', fontsize=11, weight='bold')
    plt.ylabel('Clinical Performance (%)', fontsize=11, weight='bold')
    plt.title('PathVQA Multimodal Foundation Benchmark Leaderboard', fontsize=13, weight='bold')
...
def main():
    ...
    bio_m = load_metrics(results_dir / "metrics_summary_biomedclip.json")
    ...
    print_comparison_table(bio_m, pali_m, qwen_m, blip_m)
    plot_side_by_side_bars(bio_m, pali_m, qwen_m, blip_m, results_dir / "path_vqa_benchmark_comparison.png")
