"""
Main Entrypoint for Herlev Cervical Cytology Pathology Foundation Models with fastai.
Usage:
...
# OpenMP runtime & fastcore safety patches
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Herlev Cervical Cytology Pathology Foundation Benchmark in fastai"
    )
    parser.add_argument(
        "--config",
        type=str,
...
def main():
    ...
    experiment_name = f"herlev_fastai_{safe_backbone_tag}"
    ...
    tracker = WandbExperimentTracker(
        ...
        tags=cfg.get("wandb.tags", ["fastai", "foundation", "herlev"]),
    ...
    test_metrics, y_true, y_pred, y_prob = train_fastai_foundation_model(
        train_df=train_df,
        val_df=val_df,
        test_df=test_df,
        cfg=cfg,
    ...
    if cfg.get("save_plots", True):
        ...
        plot_confusion_matrix(
            ...
            y_pred,
            cm_path,
            title=f"fastai {safe_backbone_tag.upper()} Herlev 7-Class Confusion Matrix",
        )
        ...
        plot_multiclass_roc(
            ...
            y_prob,
            roc_path,
            title=f"fastai {safe_backbone_tag.upper()} Herlev Multi-Class ROC Curves",
        )
        tracker.log_artifact(roc_path, "roc_curves")

    tracker.finish()
    print(f"\n[Completed] Herlev fastai experiment for '{backbone_name}' finished successfully!\n")
