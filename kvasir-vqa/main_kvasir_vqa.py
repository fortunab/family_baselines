"""
Main CLI Entrypoint for Gastrointestinal Endoscopy Visual Question Answering (Kvasir-VQA).
Supports 4 Foundation Models:
1. florence2 (Microsoft Florence-2)
...
from hf_kvasir_vqa_dataset import create_kvasir_vqa_dataloaders, setup_random_seed
from vqa_metrics import evaluate_vqa_predictions, print_vqa_report, plot_vqa_diagnostic_card
from model_florence2_vqa import Florence2VQAModel
...
def parse_args():
    parser = argparse.ArgumentParser(description="Gastrointestinal Endoscopy (Kvasir-VQA) Foundation Benchmark")
    parser.add_argument("--data-dir", type=str, default="./data", help="Data directory")
    parser.add_argument("--results-dir", type=str, default="./results", help="Results directory")
    parser.add_argument(
        "--model-name", type=str, default="florence2",
        choices=["florence2", "paligemma", "qwen2vl", "blip2", "all"],
...
def get_vqa_model(model_name: str):
    ...
    if "florence" in name_lower:
        return Florence2VQAModel(), "florence2"
...
def run_vqa_pipeline(model_key: str, args, project_root: Path, active_seed: int):
    ...
    print("\n" + "="*95)
    print(f"  STARTING KVASIR-VQA FOUNDATION EVALUATION: {model_key.upper()} (SimulaMet-HOST/Kvasir-VQA)")
    print("="*95)

    model, canonical_name = get_vqa_model(model_key)

    train_loader, val_loader, test_loader, seed, test_samples = create_kvasir_vqa_dataloaders(
        data_dir=data_dir,
    ...
    print(f"\n[Evaluator] Running unseen holdout test evaluation on {len(test_loader.dataset)} GI QA pairs...")
    predictions = []
    ground_truths = []
    is_closed_flags = []
...
def main():
    ...
    if args.model_name == "all":
        models = ["florence2", "paligemma", "qwen2vl", "blip2"]
        print(f"[Main] Running full Kvasir-VQA benchmark across all {len(models)} Foundation Models...")
        for m in models:
