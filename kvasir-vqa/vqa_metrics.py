"""
Clinical Endoscopy VQA Evaluation Metrics & NLP Scorer for Kvasir-VQA.
Computes:
...
6. Diagnostic Visual Endoscopy VQA Card
"""
...
def print_vqa_report(metrics: Dict[str, Any], model_name: str, seed: Optional[int] = None):
    print("\n" + "="*95)
    print(f"      KVASIR-VQA FOUNDATION MODEL EVALUATION REPORT: {model_name.upper()}")
    print("="*95)
    if seed is not None:
...
def plot_vqa_diagnostic_card(
    ...
    plt.subplot(1, 2, 1)
    plt.imshow(image)
    plt.title("Endoscopy Colonoscopy Frame", weight='bold', fontsize=11)
    plt.axis("off")
    ...
    plt.text(
        ...
        fontsize=10,
        va='center',
        bbox=dict(boxstyle="round,pad=0.8", facecolor="#f5f7fa", edgecolor="#27ae60", lw=1.5)
    )
