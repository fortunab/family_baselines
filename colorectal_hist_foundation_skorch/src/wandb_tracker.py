"""
Weights & Biases (W&B) Experiment Tracking & skorch Callback for Pathology Foundation Models.
"""
...
class WandbExperimentTracker:
    def __init__(
        self,
        project_name: str = "colorectal-histology-pathology-foundation-skorch",
        experiment_name: str = "foundation_skorch_run",
        ...
        self.tags = tags or ["skorch", "foundation", "histology", "toml"]
...
class SkorchWandbCallback(Callback):
    """
    skorch callback that intercepts epoch metrics and sends them to the WandbExperimentTracker.
    """

    def __init__(self, tracker: WandbExperimentTracker):
    ...
    def on_epoch_end(self, net, dataset_train=None, dataset_valid=None, **kwargs):
        ...
        epoch_idx = last_epoch.get("epoch", len(history))

        epoch_metrics = {
            "epoch": epoch_idx,
            "train_loss": float(last_epoch.get("train_loss", 0.0)),
            "valid_loss": float(last_epoch.get("valid_loss", 0.0)),
            "valid_acc": float(last_epoch.get("valid_acc", last_epoch.get("val_acc", 0.0))),
            "dur": float(last_epoch.get("dur", 0.0)),
        }
        if "lr" in last_epoch:
            epoch_metrics["lr"] = float(last_epoch["lr"])

        self.tracker.log_metrics(epoch_metrics, step=epoch_idx)
