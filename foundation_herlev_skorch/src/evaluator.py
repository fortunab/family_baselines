"""
Evaluation Metrics & Visualization Suite for Herlev skorch Pathology Foundation Models.
Calculates Accuracy, Balanced Accuracy, Precision, Recall, Macro/Weighted F1, ROC-AUC.
"""
...
SHORT_CLASS_NAMES = ["N.Sup", "N.Int", "N.Col", "Light", "Mod", "Sev", "CIS"]
...
def print_evaluation_report(metrics: Dict[str, Any], model_name: str, seed: Optional[int] = None):
    print("\n" + "=" * 95)
    print(f"      HERLEV CYTOLOGY PATHOLOGY FOUNDATION REPORT: {model_name.upper()}")
    print("=" * 95)
    if seed is not None:
...
def plot_confusion_matrix(
    ...
    y_pred: np.ndarray,
    output_path: Path,
    title: str = "skorch Foundation Confusion Matrix",
):
    ...
    plt.xlabel("Predicted Cytology Class", fontsize=11, weight="bold")
    plt.ylabel("Ground Truth Class", fontsize=11, weight="bold")
    plt.title(title, fontsize=12, weight="bold")
    plt.tight_layout()
    plt.savefig(output_path, dpi=300)
    plt.close()
    print(f"[Metrics] Confusion matrix heatmap saved to: {output_path}")
...
def plot_multiclass_roc(
    ...
    y_prob: np.ndarray,
    output_path: Path,
    title: str = "skorch Foundation Multi-Class ROC",
):
    ...
    for i, short_name in enumerate(SHORT_CLASS_NAMES):
