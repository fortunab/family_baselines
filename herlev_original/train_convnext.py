"""
ConvNeXt Training Engine for Original MDE-Lab Herlev Cytology.
"""
...
class ConvNeXtTrainer:
    def __init__(
        ...
        param_groups = get_convnext_parameter_groups(model=self.model, backbone_lr=backbone_lr, head_lr=head_lr, weight_decay=weight_decay)
    ...
    def fit(self, epochs: int = 15, warmup_epochs: int = 3) -> Dict[str, Any]:
        ...
        best_val_f1 = 0.0
        best_ckpt = self.output_dir / "best_convnext_model.pth"

        print(f"[ConvNeXt-Trainer] Training for {epochs} epochs on {self.device} (Seed: {self.active_seed})...")
        ...
        self.plot_curves(self.output_dir / "training_curves_convnext.png")
