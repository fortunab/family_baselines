"""
Main Execution Script for Vision Transformer / EVA-02 Baseline on Original MDE-Lab Herlev Cytology.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Vision Transformer Fine-Tuning on Original MDE-Lab Herlev")
...
def main():
    ...
    print("="*80)
    print(f"  MDE-LAB HERLEV BASELINE 3: VISION TRANSFORMER ({args.model_name}) FINE-TUNING (70/15/15 SPLIT)")
    print("="*80)

    model, cfg = create_vit_model(model_name=args.model_name, num_classes=7, pretrained=True)
    ...
    plot_roc_curves(y_true=res["y_true_test"], y_proba=res["y_proba_test"], class_names=class_names, output_path=results_dir / "roc_curves_vit.png", title=f"ViT ({args.model_name}) - 15% Test ROC Curves")
