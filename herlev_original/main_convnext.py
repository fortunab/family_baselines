"""
Main Execution Script for ConvNeXt Baseline on Original MDE-Lab Herlev Cytology.
"""
...
from mde_dataset import create_mde_dataloaders, CLASS_DESCRIPTIONS
...
def parse_args():
    parser = argparse.ArgumentParser(description="ConvNeXt Fine-Tuning on Original MDE-Lab Herlev Cytology")
...
def main():
    ...
    print("="*80)
    print(f"  MDE-LAB HERLEV BASELINE 2: CONVNEXT ({args.model_name.upper()}) FINE-TUNING (70/15/15 SPLIT)")
    print("="*80)

    model, cfg = create_convnext_model(model_name=args.model_name, num_classes=7, pretrained=True)
    img_size = args.img_size

    train_l, val_l, test_l, class_names, seed = create_mde_dataloaders(
    ...
    plot_confusion_matrix(test_metrics["Confusion_Matrix"], class_names=class_names, output_path=results_dir / "confusion_matrix_convnext.png", title=f"ConvNeXt ({args.model_name}) - 15% Test Confusion Matrix")
    plot_roc_curves(y_true=res["y_true_test"], y_proba=res["y_proba_test"], class_names=class_names, output_path=results_dir / "roc_curves_convnext.png", title=f"ConvNeXt ({args.model_name}) - 15% Test ROC Curves")
