"""
Original MDE-Lab Herlev 4-Tier Benchmark Comparison Script.
Compares:
Tier 1: Handcrafted Cytology Features + RBF-SVM
Tier 2: Modern ConvNeXt CNN (ConvNeXt-Tiny/Small)
Tier 3: SOTA Vision Transformer (ViT-Base / EVA-02)
Tier 4: Pathology & Vision Foundation (Owkin Phikon / Paige Virchow / DINOv2)
"""
...
def print_comparison_table(svm_m, cnx_m, vit_m, fnd_m):
    print("\n" + "="*125)
    print("       ORIGINAL MDE-LAB HERLEV CERVICAL CYTOLOGY BENCHMARK COMPARISON: 4-TIER LEADERBOARD")
    print("="*125)
    ...
    print(f"{'Cytology Class':<28} | {'SVM F1':<18} | {'ConvNeXt F1':<16} | {'ViT F1':<18} | {'Foundation F1':<20}")
    print("-"*125)

    available = [m for m in [svm_m, cnx_m, vit_m, fnd_m] if m is not None]
    if available:
        ref = available[0]
        ...
        for c in classes:
            ...
            f_f1 = fnd_m.get("Per_Class_Metrics", {}).get(c, {}).get("F1_Score") if fnd_m else None
            ...
            f_str = f"{f_f1*100:.2f}%" if f_f1 is not None else "N/A"
            short_c = c.split()[0] if len(c) > 26 else c
            print(f"{short_c:<28} | {s_str:<18} | {c_str:<16} | {v_str:<18} | {f_str:<20}")
...
def plot_side_by_side_bars(svm_m, cnx_m, vit_m, fnd_m, output_path: Path):
    ...
    plt.bar(x + width, aucs, width, label='ROC-AUC (%)', color='#937860')

    plt.xlabel('Paradigm', fontsize=11, weight='bold')
    plt.ylabel('Score (%)', fontsize=11, weight='bold')
    plt.title('Original MDE-Lab Herlev Benchmark Comparison (70/15/15 Split)', fontsize=13, weight='bold')
...
def main():
    ...
    plot_side_by_side_bars(svm_m, cnx_m, vit_m, fnd_m, results_dir / "mde_herlev_benchmark_comparison.png")
