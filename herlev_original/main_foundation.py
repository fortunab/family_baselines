"""
Main Execution Script for Foundation Model Baseline on Original MDE-Lab Herlev.
...
from mde_dataset import load_mde_dataset_paths_and_labels, MDECytologyDataset, get_mde_transforms, CLASS_DESCRIPTIONS, setup_random_seed
...
def parse_args():
    parser = argparse.ArgumentParser(description="Pathology/Vision Foundation Model on Original MDE-Lab Herlev")
    parser.add_argument("--data-dir", type=str, default="./data")
    parser.add_argument("--cache-file", type=str, default="./cache/features_mde_foundation.npz")
...
def main():
    ...
    print("="*80)
    print(f"  MDE-LAB HERLEV BASELINE 4: FOUNDATION MODEL ({args.model_name}) + LINEAR PROBE (70/15/15 SPLIT)")
    print("="*80)
    ...
    if cache_file.exists() and not args.force_recompute:
        ...
        tf = get_mde_transforms(img_size=cfg["img_size"], is_training=False, mean=cfg["mean"], std=cfg["std"])
        ds = MDECytologyDataset(image_paths, labels, transform=tf)
