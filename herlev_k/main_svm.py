"""
Main Execution Script for Classical SVM Baseline on Herlev Cervical Cytology.
...
from herlev_dataset import load_herlev_dataset_paths_and_labels, CLASS_NAMES, CLASS_DESCRIPTIONS, setup_random_seed
from feature_extractor import extract_herlev_features_parallel
from train_svm import train_herlev_svm_split
from evaluate import compute_all_metrics, print_evaluation_report, plot_confusion_matrix, plot_roc_curves


def parse_args():
    parser = argparse.ArgumentParser(description="Classical Handcrafted + SVM Baseline on Herlev Cytology")
    parser.add_argument("--data-dir", type=str, default="./data", help="Dataset directory")
    parser.add_argument("--cache-file", type=str, default="./cache/features_herlev_svm.npz", help="Cache path")
...
def main():
    ...
    print("="*80)
    print("  HERLEV BASELINE 1: HANDCRAFTED TEXTURE & COLOR + RBF-SVM (70/15/15 SPLIT)")
    print("="*80)
    ...
    X, y = extract_herlev_features_parallel(
        image_paths=image_paths, labels=labels, cache_path=cache_file, force_recompute=args.force_recompute
    )

    y_test_true, y_test_pred, y_test_proba, model, seed = train_herlev_svm_split(
