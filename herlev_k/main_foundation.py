"""
Main Execution Script for Foundation Model Baseline on Herlev Cervical Cytology.
...
from herlev_dataset import load_herlev_dataset_paths_and_labels, HerlevCytologyDataset, get_cytology_transforms, CLASS_DESCRIPTIONS, setup_random_seed
...
def parse_args():
    parser = argparse.ArgumentParser(description="Pathology/Vision Foundation Model on Herlev Cytology")
    parser.add_argument("--data-dir", type=str, default="./data")
    parser.add_argument("--cache-file", type=str, default="./cache/features_herlev_foundation.npz")
    parser.add_argument("--results-dir", type=str, default="./results")
    parser.add_argument("--model-name", type=str, default="owkin/phikon")
...
def main():
    ...
    print("="*80)
    print(f"  HERLEV BASELINE 4: FOUNDATION MODEL ({args.model_name}) + LINEAR PROBE (70/15/15 SPLIT)")
    print("="*80)
    ...
    if cache_file.exists() and not args.force_recompute:
        ...
        tf = get_cytology_transforms(img_size=cfg["img_size"], is_training=False, mean=cfg["mean"], std=cfg["std"])
        ds = HerlevCytologyDataset(image_paths, labels, transform=tf)
    ...
    plot_roc_curves(y_true=y_test_true, y_proba=y_test_proba, class_names=class_names, output_path=results_dir / "roc_curves_foundation.png", title=f"Foundation ({args.model_name.split('/')[-1]}) - 15% Test ROC Curves")
