"""
Main Execution Script for ConvNeXt Baseline on Herlev Cervical Cytology.
"""
...
from herlev_dataset import create_herlev_dataloaders, CLASS_DESCRIPTIONS
...
def parse_args():
    parser = argparse.ArgumentParser(description="ConvNeXt Fine-Tuning on Herlev Cytology")
...
def main():
    ...
    print("="*80)
    print(f"  HERLEV BASELINE 2: CONVNEXT ({args.model_name.upper()}) FINE-TUNING (70/15/15 SPLIT)")
    print("="*80)

    model, cfg = create_convnext_model(model_name=args.model_name, num_classes=7, pretrained=True)
    img_size = args.img_size

    train_l, val_l, test_l, class_names, seed = create_herlev_dataloaders(
