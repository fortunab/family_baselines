"""
CLI Entrypoint for fastai Colorectal Histology Foundation Benchmark.
Supports:
...
- Dynamic random seeds and holdout test set evaluation.
"""
...
from models.fastai_pipeline import train_and_eval_fastai


def parse_cli_args():
    parser = argparse.ArgumentParser(description="fastai Colorectal Histology Foundation Benchmark")
    parser.add_argument("--config", type=str, default="configs/default_config.ini", help="Path to config file (.ini, .toml, .json)")
    parser.add_argument("--backbone", type=str, default=None, help="Backbone model (e.g. convnext_base, vit_base_patch16_224, efficientnet_b3)")
...
def main():
    ...
    exp_name = f"fastai_{cfg.get('backbone', 'convnext_base')}_{active_seed}"
    tracker = ExperimentTracker(
        ...
        tags=str(cfg.get("tags", "fastai,histology"))
    )

    # 4. Train & Evaluate with fastai
    metrics, y_true, y_pred, y_prob = train_and_eval_fastai(
        train_df=train_df,
        val_df=val_df,
        test_df=test_df,
        cfg=cfg,
    ...
    cm_path = results_dir / f"confusion_matrix_fastai_{cfg.get('backbone', 'model')}.png"
    roc_path = results_dir / f"roc_curves_fastai_{cfg.get('backbone', 'model')}.png"
    json_path = results_dir / f"metrics_fastai_{cfg.get('backbone', 'model')}.json"

    plot_confusion_matrix(y_true, y_pred, output_path=cm_path, title=f"fastai {cfg.get('backbone')} Confusion Matrix")
    plot_multiclass_roc(y_true, y_prob, output_path=roc_path, title=f"fastai {cfg.get('backbone')} Multi-Class ROC")
