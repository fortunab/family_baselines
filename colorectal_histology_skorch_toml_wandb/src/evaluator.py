"""
Evaluation Metrics & Visualization Suite for skorch Colorectal Histology.
...
def print_evaluation_report(metrics: Dict[str, Any], model_name: str, seed: Optional[int] = None):
    print("\n" + "=" * 95)
    print(f"      COLORECTAL HISTOLOGY SKORCH + W&B REPORT: {model_name.upper()}")
    print("=" * 95)
    if seed is not None:
...
def plot_confusion_matrix(
    ...
    y_pred: np.ndarray,
    output_path: Path,
    title: str = "skorch Confusion Matrix",
):
...
def plot_multiclass_roc(
    ...
    y_prob: np.ndarray,
    output_path: Path,
    title: str = "skorch Multi-Class ROC Curves",
):
