"""
Fine-Tuning Engine for Vision Transformers (EVA-02 / ViT / Swin).
...
from vit_models import get_parameter_groups
from evaluate import compute_all_metrics


class ViTTrainer:
    """
    Manages fine-tuning, validation, evaluation, and checkpointing for Vision Transformers.
    """
    def __init__(
        self,
        model: nn.Module,
    ...
    def fit(
        ...
        """
        Executes full fine-tuning schedule with Cosine Annealing learning rate schedule.
        """
        ...
        best_val_f1 = 0.0
        best_checkpoint_path = self.output_dir / "best_vit_model.pth"

        print(f"[ViT-Trainer] Starting Fine-Tuning for {epochs} epochs (Warmup: {warmup_epochs} epochs)...")
        print(f"[ViT-Trainer] Target Device: {self.device} (Mixed Precision AMP: {self.use_amp})")
        ...
        print(f"[ViT-Trainer] Fine-tuning completed in {elapsed/60:.2f} minutes. Best Val F1: {best_val_f1*100:.2f}%")

        # Load best checkpoint for final test evaluation
        print(f"[ViT-Trainer] Loading best checkpoint from {best_checkpoint_path} for test set evaluation...")
        ...
        # Plot training loss & accuracy progression curves
        self.plot_training_curves(self.output_dir / "training_curves_vit.png")
    ...
    def plot_training_curves(self, output_path: Path):
        ...
        plt.title("Cross-Entropy Loss vs. Epochs", fontsize=12, weight='bold')
        ...
        plt.title("Accuracy (%) vs. Epochs", fontsize=12, weight='bold')
        plt.xlabel("Epoch", fontsize=10, weight='bold')
        ...
        plt.close()
        print(f"[ViT-Trainer] Training curves plot saved to: {output_path}")
