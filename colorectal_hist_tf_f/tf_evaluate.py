"""
TensorFlow Evaluation & Diagnostic Visualization Module for Colorectal Histology.
Computes 8-Class metrics: Accuracy, Balanced Accuracy, Macro/Weighted F1,
Precision, Recall, Specificity, Cohen's Kappa, MCC, Macro ROC-AUC, Confusion Matrix, and ROC curves.
"""
...
def compute_all_metrics(
    ...
    if class_names is None:
        class_names = [f"Class_{i}" for i in range(max(8, n_classes))]
...
def print_evaluation_report(metrics: Dict[str, Any], class_names: List[str]):
    print("\n" + "="*95)
    print("      TENSORFLOW COLORECTAL HISTOLOGY FOUNDATION BENCHMARK EVALUATION REPORT")
    print("="*95)
    ...
    print("-"*95)
    print(" PER-CLASS HISTOLOGY BREAKDOWN:")
    print(f" {'Tissue Class':<42} | {'Precision':<9} | {'Sens/Rec':<9} | {'Spec.':<9} | {'F1-Score':<9} | {'Support':<7}")
    print("-"*95)
    for name in class_names:
        if name in metrics["Per_Class_Metrics"]:
            pc = metrics["Per_Class_Metrics"][name]
            print(f" {name:<42} | {pc['Precision']*100:>8.2f}% | {pc['Sensitivity_Recall']*100:>8.2f}% | {pc['Specificity']*100:>8.2f}% | {pc['F1_Score']*100:>8.2f}% | {pc['Support']:>7}")
    print("="*95 + "\n")
...
def plot_confusion_matrix(cm: np.ndarray, class_names: List[str], output_path: Path, title: str = "Normalized Confusion Matrix"):
    ...
    sns.heatmap(cm_norm, annot=annot, fmt="", cmap="Blues", xticklabels=short_names, yticklabels=short_names, cbar=True)
    plt.title(title, fontsize=12, weight='bold', pad=12)
    plt.xlabel("Predicted Histology Class", fontsize=11, weight='bold')
    plt.ylabel("True Histology Class", fontsize=11, weight='bold')
    plt.tight_layout()
