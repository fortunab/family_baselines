"""
Evaluation & Diagnostic Visualization Module for CVC-Colon Endoscopy.
Computes Binary Classification metrics: Accuracy, Sensitivity (Recall), Specificity,
Precision, F1-Score, Cohen's Kappa, Matthews Correlation Coefficient (MCC), ROC-AUC, PR-AUC, Confusion Matrix, ROC curves, and Precision-Recall curves.
"""
...
def compute_all_metrics(
    ...
    if y_proba is not None:
        try:
            p_pos = y_proba[:, 1] if y_proba.ndim == 2 and y_proba.shape[1] == 2 else y_proba
...
def print_evaluation_report(metrics: Dict[str, Any], class_names: List[str]):
    print("\n" + "="*85)
    print("           CVC-COLON COLONOSCOPY BENCHMARK EVALUATION REPORT")
    print("="*85)
    ...
    if "ROC_AUC" in metrics:
        print(f" ROC-AUC (Area)         : {metrics['ROC_AUC']*100:.2f}%")
    if "PR_AUC" in metrics:
        print(f" PR-AUC (Precision-Rec) : {metrics['PR_AUC']*100:.2f}%")
    print("-"*85)
...
def plot_roc_curves(y_true: np.ndarray, y_proba: np.ndarray, output_path: Path, title: str = "CVC-Colon ROC Curve"):
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    auc_val = roc_auc_score(y_true, p_pos)

    plt.plot(fpr, tpr, color='#1f77b4', lw=2.2, label=f'ROC Curve (AUC = {auc_val:.4f})')
    plt.plot([0, 1], [0, 1], 'k--', lw=1.5, alpha=0.7, label='Chance (AUC = 0.500)')
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
