"""
CVC-Colon 4-Tier Benchmark Comparison Script.
Compares:
Tier 1: Handcrafted Endoscopy Features + RBF-SVM
...
def print_comparison_table(svm_m, cnx_m, vit_m, fnd_m):
    print("\n" + "="*125)
    print("                 CVC-COLON COLONOSCOPY BENCHMARK COMPARISON: 4-TIER LEADERBOARD")
    print("="*125)
    ...
    metrics_to_compare = [
        ...
        ("ROC-AUC (Area)", "ROC_AUC", True),
        ("PR-AUC (Precision-Rec)", "PR_AUC", True)
    ]
    ...
    print(f"{'Class':<28} | {'SVM F1':<18} | {'ConvNeXt F1':<16} | {'ViT F1':<18} | {'Foundation F1':<20}")
    print("-"*125)
    ...
    if available:
        ...
        for c in classes:
            ...
            print(f"{c:<28} | {s_str:<18} | {c_str:<16} | {v_str:<18} | {f_str:<20}")
...
def plot_side_by_side_bars(svm_m, cnx_m, vit_m, fnd_m, output_path: Path):
    ...
    plt.title('CVC-Colon Benchmark Comparison (70/15/15 Split)', fontsize=13, weight='bold')
...
def main():
    ...
    plot_side_by_side_bars(svm_m, cnx_m, vit_m, fnd_m, results_dir / "cvc_colon_benchmark_comparison.png")
