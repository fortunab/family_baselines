"""
Handcrafted Feature Extractor for CVC-Colon Endoscopy Frames.
Extracts:
1. Multiscale Uniform LBP (Mucosal vascular surface texture)
...
class CVCEndoscopyFeatureExtractor:
    def __init__(
        self,
...
def extract_cvc_features_parallel(
    ...
    extractor = CVCEndoscopyFeatureExtractor()
    print(f"[FeatureExtractor] Extracting handcrafted features on {len(image_paths)} CVC colonoscopy frames...")
    ...
    results = Parallel(n_jobs=n_jobs, batch_size=64)(
        delayed(process)(p) for p in tqdm(image_paths, desc="Extracting CVC Descriptors")
    )
